...
export default function StaffFirstLogin({ session, staff }) {
  ...
  const sendOtp = async () => {
    ...
    setSending(false);
    if (err) {
      setError("Could not send code: " + err.message);
    } else {
      setSent(true);
      setCooldown(60);
    }
  ...
  const setNewPassword = async () => {
    ...
    // Mark password_changed in staff table
    if (staff?.id) {
      await supabase.from("staff").update({ password_changed: true }).eq("id", staff.id);
    }

    setSuccess(true);
    setSaving(false);

    // useAuth will auto-detect the cleared must_change_password and redirect to staff dashboard
    setTimeout(() => supabase.auth.getSession(), 500);
  };
  ...
  if (success) return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center px-6">
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Signing you in to your dashboard…</p>
        ...
        {step === "otp" && (
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] text-slate-400 font-semibold ml-1">Step 1 of 2</p>
            </div>

            {sent
              ? <p className="text-xs text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl px-4 py-2.5 font-medium">
                  ✓ Code sent — check your email inbox (and spam folder)
                </p>
              : sending
              ? <p className="text-xs text-slate-400 dark:text-slate-500 text-center">Sending code…</p>
              : null
            }
        ...
        {step === "password" && (
          <>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 rounded-full bg-green-500" />
              <div className="flex-1 h-1.5 rounded-full bg-green-500" />
              <p className="text-[11px] text-slate-400 font-semibold ml-1">Step 2 of 2</p>
            </div>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl px-4 py-3">
              <p className="text-xs text-green-700 dark:text-green-300 font-medium">
                ✓ Email verified — now set your permanent password
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/60 rounded-xl px-4 py-3 space-y-1.5">
                <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Password must have</p>
                {[
                  ...
                  <div key={rule} className="flex items-center gap-2">
                    ...
                    <p className={`text-[11px] font-medium ${pass ? "text-green-600 dark:text-green-400" : "text-slate-400 dark:text-slate-500"}`}>{rule}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {staff && (
          <div className="flex items-center gap-3 bg-white dark:bg-slate-800 rounded-2xl px-4 py-3 border border-slate-100 dark:border-slate-700">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 capitalize truncate">{staff.role?.replace(/_/g, " ")}</p>
            </div>
            <button
