...
function RequestOrgArchiveSection({ org, onRequested }) {
  ...
  const checkBlockers = async () => {
    ...
    try {
      const r = await coopFn("get_org_archive_blockers", { org_id: org.id });
      const bl = r.blockers ?? [];
      setBlockers(bl);
      setPhase(bl.length > 0 ? "blocked" : "ready");
  ...
  if (phase === "idle") {
    return (
      <button onClick={checkBlockers}
        className="w-full py-2.5 rounded-xl border border-red-200 dark:border-red-900/40 text-xs font-bold text-red-500 dark:text-red-400 active:bg-red-50 dark:active:bg-red-900/20 transition-colors">
        Request Organisation Archive
      </button>
    );
  }
  ...
  if (phase === "blocked") {
    return (
      <div className="space-y-3">
        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800/40 p-3 space-y-2">
          <p className="text-[11px] font-bold text-red-600 dark:text-red-400 uppercase tracking-wide">Must settle before archiving</p>
          {blockers.map(b => (
            <div key={b.type} className="flex items-center justify-between text-xs">
              <span className="text-red-700 dark:text-red-300">{b.label}: <span className="font-bold">{b.count}</span></span>
              {b.total != null && <span className="font-bold text-red-700 dark:text-red-300">{fmt(b.total)}</span>}
            </div>
          ))}
          <p className="text-[10px] text-red-500 dark:text-red-400 pt-1">Complete all outstanding items above, then check again.</p>
        </div>
        <button onClick={checkBlockers}
          className="w-full py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-500">
          Check again
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800/40 px-3 py-2">
        ...
        <p className="text-[11px] text-green-700 dark:text-green-400 font-semibold">All safety checks passed — ready to submit</p>
      </div>
    ...
    <div className="flex-1 overflow-y-auto px-4 pt-4 pb-24 flex flex-col gap-5">
      ...
      {!isOrgPortal && <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-red-100 dark:border-red-900/40">
        ...
        {org.status === "archived" ? (
          ...
          <>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
              Submit a request to archive this organisation. All member records and transaction history remain intact and reportable. Operations lock while the request is under review.
            </p>
            <RequestOrgArchiveSection org={org} onRequested={() => { onOrgUpdate(p => ({ ...p, pending_archive: true })); onRefresh(); }} />
          </>
        )}
      </div>}
      ...
      <div className="h-[100dvh] bg-slate-50 dark:bg-slate-900 flex justify-center transition-colors duration-200">
        <div className="w-full max-w-md flex flex-col h-full relative">
          ...
          <header className="flex-none z-30 min-h-[56px] flex items-center justify-between px-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 shadow-sm" style={{ paddingTop: "max(12px, env(safe-area-inset-top, 12px))" }}>
            <img src="/icon-transparent.png" alt="KudiAi" className="h-8 w-8 object-contain flex-shrink-0" />
