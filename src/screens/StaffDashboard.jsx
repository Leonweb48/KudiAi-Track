...
export default function StaffDashboard({ session, staff }) {
  ...
  // "bills" is always pinned in the nav so don't evict it — it shows a no-access message instead.
  useEffect(() => {
    if (tab !== "overview" && tab !== "profile" && tab !== "bills" && !allowed.includes(tab)) {
      setTab("overview");
    }
  ...
  const renderTab = () => {
    ...
    if (tab !== "overview" && tab !== "profile" && tab !== "bills" && !allowed.includes(tab)) return null;
    // Bills is always reachable but shows a no-access screen when permission is off
    if (tab === "bills" && !allowed.includes("bills")) {
      return (
        <div className="flex flex-col items-center justify-center px-6 py-24 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Your manager has not enabled bill payments for your account. Ask them to turn it on in Staff Management.</p>
        </div>
      );
    }
        ...
        <div className="w-full max-w-md relative flex flex-col h-screen">
          ...
          <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-40 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-float">
            <div className="flex items-stretch h-[60px]">
              {["overview", "bills", ...allowed.filter(m => m !== "bills"), "profile"].map(m => {
