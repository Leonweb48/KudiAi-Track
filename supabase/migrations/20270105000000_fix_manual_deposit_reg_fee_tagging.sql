-- ═════════════════════════════════════════════════════════════════════════════
-- ajo_confirm_manual_deposit's registration-fee INSERT never carried
-- contribution_context/cycle_id/group_id at all — unlike its Paystack-path
-- twin in the same file, ajo_approve_contribution, whose equivalent insert
-- correctly propagates all three from the parent contribution row.
--
-- v_is_first (a client's very first-ever completed contribution) isn't
-- scoped to personal_savings — a client whose first-ever deposit happens to
-- be a manual-transfer claim into a group_savings or esusu_rotation entity
-- gets a registration-fee row silently defaulted to
-- contribution_context='personal_savings', cycle_id=NULL, group_id=NULL.
--
-- This also affects the PERSONAL-CYCLE case specifically (not just group/
-- esusu): ajo_entity_stats' cycle branch filters its fee sum and its locked
-- sum by cycle_id directly — a reg-fee row stuck at cycle_id=NULL never
-- matches, so a fee that really was deducted from current_balance never
-- reduces that cycle's own "available" figure.
--
-- Now carries contribution_context/cycle_id/group_id straight from the claim
-- row being confirmed (v_claim) — the same row ajo_submit_manual_claim
-- already tagged correctly at submission time.
-- ═════════════════════════════════════════════════════════════════════════════
...
-- ── One-time repair: already-mistagged manual-deposit registration fees ──────
-- Safe signature: a registration_fee row still at the column default
-- ('personal_savings', cycle_id NULL, group_id NULL) whose parent claim (via
-- fee_for_contribution_id) says otherwise. ajo_approve_contribution's own
-- registration-fee rows are already correctly tagged, so they never match
-- this signature — only rows this specific bug could have produced do.
DO $$
DECLARE
  v_fixed INT;
BEGIN
  UPDATE ajo_contributions fee
  SET contribution_context = COALESCE(orig.contribution_context, 'personal_savings'),
      cycle_id             = orig.cycle_id,
      group_id             = orig.group_id
  FROM ajo_contributions orig
  WHERE orig.id = fee.fee_for_contribution_id
    AND fee.type = 'registration_fee'
    AND fee.payment_method = 'manual_transfer'
    AND fee.contribution_context = 'personal_savings'
    AND fee.cycle_id IS NULL
    AND fee.group_id IS NULL
    AND (orig.contribution_context <> 'personal_savings' OR orig.cycle_id IS NOT NULL OR orig.group_id IS NOT NULL);

  GET DIAGNOSTICS v_fixed = ROW_COUNT;
  RAISE NOTICE 'manual-deposit registration_fee repair: retagged % row(s)', v_fixed;
END $$;
