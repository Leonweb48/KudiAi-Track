-- ═════════════════════════════════════════════════════════════════════════════
-- ajo_record_withdrawal never set contribution_context on the withdrawal (or
-- withdrawal_fee) rows it inserts — it silently defaulted to the column's
-- 'personal_savings' default, regardless of which entity the withdrawal was
-- actually attributed to (cycle_id / group_id).
--
-- Both getGroupStats (client, AjoMemberPortal.jsx) and ajo_entity_stats
-- (server) compute a group/esusu circle's "available" by filtering
-- ajo_contributions strictly on contribution_context = 'esusu_rotation' (or
-- 'group_savings') — INCLUDING the withdrawal rows they subtract. A
-- withdrawal mistagged 'personal_savings' is invisible to that subtraction,
-- so "available" for that circle never decreases no matter how many
-- withdrawals are actually made against it. This is what a client saw as a
-- withdrawal card frozen at its original payout amount after two successful
-- withdrawals against it: the payout was correctly tagged, the withdrawals
-- weren't, so nothing ever got subtracted.
--
-- This was invisible for personal-cycle withdrawals specifically because
-- 'personal_savings' happens to already be the correct tag there — the bug
-- only ever showed up for group/esusu withdrawals, which is exactly the
-- surface that's been under the microscope this session.
--
-- Not a data-safety issue: current_balance itself was always decremented
-- correctly (a separate, unconditional UPDATE), and the withdrawal amount
-- was always still correctly checked against it before this fix — only the
-- PER-ENTITY figures (what's shown per-circle, and what ajo_entity_stats
-- reports) were wrong. No repair of past rows is included here: a targeted
-- backfill needs a case-by-case read of each row's actual group_id/cycle_id
-- (already present on the row) rather than a blind bulk UPDATE — safer to
-- do deliberately if/when a specific stuck balance is reported, the same way
-- the esusu-payout repair earlier in this session was scoped precisely
-- rather than guessed.
-- ═════════════════════════════════════════════════════════════════════════════
...
-- ── One-time repair: already-mistagged withdrawal/fee rows ───────────────────
-- Safe signature: only rows the OLD function could have produced — type is a
-- withdrawal/fee, still carrying the column default 'personal_savings', but
-- WITH a group_id set (which the old code did set correctly — only the
-- context was ever missing). A genuine personal-cycle withdrawal has no
-- group_id and is correctly left untouched.
DO $$
DECLARE
  v_fixed INT;
BEGIN
  UPDATE ajo_contributions ac
  SET contribution_context = CASE
    WHEN g.group_mode = 'rotating' THEN 'esusu_rotation'
    ELSE 'group_savings'
  END
  FROM ajo_groups g
  WHERE g.id = ac.group_id
    AND ac.type IN ('withdrawal', 'withdrawal_fee')
    AND ac.contribution_context = 'personal_savings'
    AND ac.group_id IS NOT NULL;

  GET DIAGNOSTICS v_fixed = ROW_COUNT;
  RAISE NOTICE 'withdrawal contribution_context repair: retagged % row(s)', v_fixed;
END $$;
