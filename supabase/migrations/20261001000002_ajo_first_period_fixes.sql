-- ─────────────────────────────────────────────────────────────────────────────
-- Ajo first-period: 5 comprehensive fixes
--
--  1. ajo_approve_contribution:
--     a) v_is_first now uses contribution count (not total_saved=0)
--        so that a first-period Day 1 deposit (net=0) doesn't mistakenly
--        charge reg fee again on Day 2.
--     b) total_saved += v_net_add (not v_contrib.amount) so the collector's
--        fee never appears in the client's savings total.
--
--  2. ajo_confirm_manual_deposit:
--     Full first-period commission logic added (was missing entirely).
--     Same v_is_first and total_saved fixes as above.
--
--  3. ajo_reverse_contribution:
--     a) Reversal net row now copies cycle_id from the original so that
--        ContributionCard's cycleContribs filter can see the reversal.
--     b) When a 'commission' fee row is reversed, commission_balance on
--        the cycle is reset to 0 so the UI correctly un-marks period 0.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Registration fee: first-ever completed contribution only.
  -- Use contribution count (not total_saved) so a first-period Day 1 deposit
  -- that nets to zero doesn't re-trigger the reg fee on Day 2.
  ...
  -- Net balance credit: deposit minus reg fee minus collector's fee.
  -- total_saved uses the same v_net_add so the collector's Day 1 fee never
  -- inflates the client's savings total.
...
-- ── 2. ajo_confirm_manual_deposit — add first-period logic ────────────────────
...
BEGIN
  ...
  -- First-deposit check via contribution count (not total_saved)
  v_is_first := NOT EXISTS (
    SELECT 1 FROM ajo_contributions
  ...
  -- ── Collector's fee (first_period cycles, manual transfer path) ──────────
...
-- ── 3. ajo_reverse_contribution — copy cycle_id + reset commission_balance ────
...
BEGIN
  ...
  -- Insert reversal row — copy cycle_id from original so ContributionCard's
  -- cycleContribs filter can find this reversal and exclude the original period.
  INSERT INTO ajo_contributions (
  ...
  -- Reverse each fee row linked to the original
  FOR v_fee_rows IN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- When the collector's fee (commission) is reversed, reset commission_balance
      -- to 0 so the cycle card correctly un-marks period 0 as "Collector's".
