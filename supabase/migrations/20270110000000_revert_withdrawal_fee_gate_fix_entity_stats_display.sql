-- ═════════════════════════════════════════════════════════════════════════════
-- Corrects the previous migration (20270109000000), which was based on a
-- wrong premise. Confirmed with the owner: the percent-model commission is
-- intentionally applicable to EVERY withdrawal from a client's savings
-- balance — personal, group, or esusu — with the one carve-out being
-- first_period-model cards (a different commission mechanism entirely,
-- already excluded since this lookup only ever matches commission_model =
-- 'percent'). So charging the fee on a group/esusu withdrawal was correct
-- behavior, not a bug, and the ₦175 refund issued by 20270109000000's
-- backfill was itself wrong money.
--
-- The REAL bug: once a percent-model fee is executed at withdrawal time, it
-- must stop being counted as still "available" in that circle's own
-- saved/withdrawn breakdown. ajo_entity_stats' esusu_rotation and
-- group_savings branches never included fee rows (commission /
-- registration_fee / withdrawal_fee) in their subtraction at all — only the
-- p_cycle_id (personal) branch did — so a member who paid ₦175 in fees
-- across two esusu withdrawals still saw that exact ₦175 rendered as
-- "available" on the circle's card, even though it had already left their
-- balance. (The client-side mirror of this same formula, AjoMemberPortal.
-- jsx's getGroupStats, had the identical gap — fixed in the same commit as
-- this migration.)
--
-- This migration:
--   1. Reverts ajo_record_withdrawal to the pre-20270109000000 fee lookup
--      (unconditional — no group_id gate).
--   2. Undoes the specific 2 erroneous reversal_withdrawal_fee rows the last
--      migration's backfill inserted (identified by their exact note text —
--      nothing else in the system produces that string) and the matching
--      current_balance credit, since that money was never actually owed
--      back.
--   3. Adds the missing fee subtraction to ajo_entity_stats' esusu_rotation
--      and group_savings branches.
-- ═════════════════════════════════════════════════════════════════════════════

-- ── 1. Revert the fee lookup gate ─────────────────────────────────────────────
...
BEGIN
  ...
  -- Commission applies to every withdrawal from the client's savings balance
  -- — personal, group, or esusu alike — with the sole carve-out being
  -- first_period-model cards, which this lookup already excludes by only
  -- ever matching commission_model = 'percent'.
...
-- ── 2. Undo the previous migration's erroneous ₦175 refund ───────────────────
DO $$
DECLARE
  r       RECORD;
  v_fixed INT     := 0;
...
BEGIN
  FOR r IN
    ...
    WHERE type = 'reversal_withdrawal_fee'
      AND notes = 'Refund: fee was wrongly charged against a group/esusu withdrawal using an unrelated personal-cycle commission rate (system fix 2027-01-09)'
  LOOP
    DELETE FROM ajo_contributions WHERE id = r.id;

    UPDATE aso_clients
    SET current_balance = GREATEST(0, COALESCE(current_balance, 0) - r.amount)
  ...
  RAISE NOTICE 'undo of incorrect withdrawal-fee refund: removed % row(s), total reclaimed ₦%', v_fixed, v_total;
END $$;

-- ── 3. Fix ajo_entity_stats: subtract fees from esusu/group "available" too ──
  ...
  ELSIF p_group_id IS NOT NULL THEN
    ...
    IF v_group_mode = 'rotating' THEN
      SELECT
        ...
        COALESCE(SUM(CASE WHEN type IN ('commission', 'registration_fee', 'withdrawal_fee') THEN amount ELSE 0 END), 0)
      INTO v_saved, v_received, v_withdrawn, v_fees
      FROM ajo_contributions
