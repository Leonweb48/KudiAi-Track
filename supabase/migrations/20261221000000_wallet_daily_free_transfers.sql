-- ═════════════════════════════════════════════════════════════════════════════
-- First 3 chargeable wallet transfers each day are free — one shared quota per
-- wallet, per calendar day (Africa/Lagos), counted across BOTH internal
-- wallet-to-wallet movement (an Ajo client's contribution into their
-- collector's wallet, and the owner's side of a scheduled withdrawal payout)
-- AND external transfers out to a bank. From the 4th qualifying transfer of
-- the day, a fee applies:
--   - External transfers: unchanged fee mechanics (Flutterwave's real
--     transfer fee + the CBN levy on amounts ≥ ₦10,000) — now gated behind
--     the free-quota instead of charging on every single transfer.
--   - Internal transfers: NEWLY charged once the quota is used — same flat
--     ₦50-on-≥₦10,000 figure as the CBN levy, but booked under a distinct
--     'wallet_fee' source/label, since no real CBN levy or bank cost is
--     actually incurred by a transfer that never leaves KudiAI's own ledger.
-- Every fee charged, either way, is credited to the platform settlement
-- wallet exactly as before.
-- ═════════════════════════════════════════════════════════════════════════════

-- ── 1. Daily transfer counter ────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.wallet_daily_transfer_count(p_user_id UUID)
RETURNS INT LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT count(*)::int FROM public.wallet_ledger
  WHERE user_id = p_user_id
    AND direction = 'debit'
    AND source IN ('ajo_contribution', 'ajo_payout', 'withdrawal')
    AND status = 'completed'
    AND (created_at AT TIME ZONE 'Africa/Lagos')::date = (now() AT TIME ZONE 'Africa/Lagos')::date;
$$;
REVOKE ALL ON FUNCTION public.wallet_daily_transfer_count(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.wallet_daily_transfer_count(UUID) TO authenticated, service_role;

-- ── 2. Widen the ledger source check for the new internal-transfer fee ─────
...
-- ── 3. wallet_pay_ajo_contribution — client -> owner. Rebuilt on the verified
--       live definition; only the new fee block (after the existing ledger
--       inserts, before the first-period commission logic) is added. ───────
...
BEGIN
  ...
  -- ── NEW: daily-free-transfer wallet fee — first 3 qualifying transfers/day
  --    (shared with external transfers) are free; from the 4th, charge the
  --    same flat figure as the external CBN levy, but as a platform 'wallet_fee'
  --    (not a CBN levy — this transfer never touches a bank). Best-effort:
  --    skipped, not blocking, if the remaining balance can't quite cover it. ──
  IF public.wallet_daily_transfer_count(v_uid) > 3 THEN
    v_wfee := public.wallet_cbn_levy_kobo(p_amount_kobo);
    IF v_wfee > 0 THEN
...
-- ── 4. ajo_settle_due_wallet_payouts — owner -> client. Rebuilt on the
--       verified live definition; only the new fee block (charged to the
--       owner, after the existing ledger inserts) is added. ────────────────
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- ── NEW: daily-free-transfer wallet fee, charged to the OWNER (whose
      --    wallet funds the payout) — same shared quota, same best-effort. ──
      IF public.wallet_daily_transfer_count(v_row.owner_id) > 3 THEN
        v_wfee := public.wallet_cbn_levy_kobo(v_row.amount_kobo);
        IF v_wfee > 0 THEN
...
-- ── 5. wallet_transfer_sent — external. Rebuilt on the verified live
--       definition; both existing fee blocks (FLW fee, CBN levy) are now
--       gated behind the same shared daily-free-transfer quota — unchanged
--       otherwise. ──────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- already includes the current transfer — matches how the internal-transfer
  -- checks above count themselves too.
  v_free := public.wallet_daily_transfer_count(v_wd.user_id) <= 3;

  -- ── Charge the real transfer fee to the same wallet, best-effort — only
  --    once the day's first 3 free transfers are used. The transfer amount
  --    itself was already held/debited at wallet_hold_transfer time (before
  --    the fee was known); this is a separate, additional debit for the fee
  --    alone, booked the moment Flutterwave reports the real number. If the
  --    wallet doesn't have quite enough left to cover a few extra naira of
  --    fee (rare — fees are small relative to the transfer just sent), this
  --    is skipped rather than blocking or reversing an already-successful
  --    transfer. ─────────────────────────────────────────────────────────
  IF NOT v_free AND COALESCE(p_fee_kobo, 0) > 0 THEN
  ...
  -- ── CBN electronic money transfer levy (₦50, transfers ≥ ₦10,000) — same
  --    free-quota gate, same best-effort balance check, independent of the
  --    FLW fee above. ────────────────────────────────────────────────────
  IF NOT v_free THEN
