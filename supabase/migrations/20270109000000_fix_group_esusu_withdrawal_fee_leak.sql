-- ═════════════════════════════════════════════════════════════════════════════
-- ajo_record_withdrawal charged a client's PERSONAL savings cycle commission
-- (commission_percent on whatever percent-model cycle happens to be their one
-- active ajo_cycles row) against ANY withdrawal, including one attributed to
-- a completely unrelated group_savings/esusu_rotation entity (v_attr_group_id
-- IS NOT NULL). ajo_groups carries no commission_model/commission_percent
-- column at all — group/esusu withdrawals have no commission concept in this
-- product — so every withdrawal_fee row ever produced against a group_id was
-- unconditionally wrong money, not merely mis-tagged.
--
-- Concretely: a client running a 3.5%-commission personal cycle who also
-- withdrew from an esusu circle had that same 3.5% silently applied to the
-- esusu withdrawal's gross amount (e.g. ₦4,000 → ₦140 fee, ₦1,000 → ₦35 fee)
-- and deducted from current_balance, even though the fee had nothing to do
-- with the esusu circle or its payout.
--
-- Fix: skip the personal-cycle commission lookup entirely once the withdrawal
-- is attributed to a group/esusu entity (v_attr_group_id IS NOT NULL) — v_pct_fee
-- stays 0, so no withdrawal_fee row is produced for that withdrawal at all.
--
-- Followed by a one-time, precisely-scoped refund: every already-completed
-- withdrawal_fee row that carries a group_id could ONLY have been produced by
-- this exact bug (ajo_record_withdrawal is the sole source of withdrawal_fee
-- rows, and a fee row is only ever inserted when the computed fee is > 0), so
-- every such row still unreversed is refunded — a reversal_withdrawal_fee
-- credit row plus the matching current_balance credit — system-wide, not
-- just for the one client who reported it.
-- ═════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_wd_context           TEXT := 'personal_savings';
  v_wd_group_mode        TEXT;
...
BEGIN
  ...
  -- Tag the withdrawal with the SAME context its source entity uses, so it's
  -- visible to that entity's own available-balance sum. Falls back to
  -- 'personal_savings' when no group is attributed (matches the column
  -- default, and is correct for a plain personal-cycle withdrawal).
  IF v_attr_group_id IS NOT NULL THEN
    SELECT group_mode INTO v_wd_group_mode FROM ajo_groups WHERE id = v_attr_group_id;
    v_wd_context := CASE WHEN v_wd_group_mode = 'rotating' THEN 'esusu_rotation' ELSE 'group_savings' END;
  END IF;

  -- Percent-model commission belongs ONLY to the client's personal savings
  -- cycle — ajo_groups has no commission_model/commission_percent column, so
  -- group_savings/esusu_rotation withdrawals have no commission concept at
  -- all. Skip the lookup entirely once this withdrawal is attributed to a
  -- group/esusu entity, so an unrelated active personal cycle's rate can
  -- never be charged against money that was never part of that cycle.
  IF v_attr_group_id IS NULL THEN
  ...
  INSERT INTO ajo_contributions (
    ...
    v_attr_cycle_id, v_attr_group_id, v_wd_context
  )
...
-- ── One-time refund: withdrawal_fee rows wrongly charged against a group ────
-- ── or esusu entity by the bug fixed above ──────────────────────────────────
DO $$
DECLARE
  r        RECORD;
  v_fixed  INT     := 0;
...
BEGIN
  FOR r IN
    ...
    WHERE ac.type = 'withdrawal_fee'
      AND ac.status = 'completed'
      AND ac.group_id IS NOT NULL
      AND ac.amount > 0
      AND NOT EXISTS (
        SELECT 1 FROM ajo_contributions rv WHERE rv.reverses_contribution_id = ac.id
      )
  LOOP
    INSERT INTO ajo_contributions (
      aso_client_id, owner_id, amount, type, payment_method,
      status, notes, recorded_by,
      reverses_contribution_id, paystack_status,
      cycle_id, group_id, contribution_context
    ) VALUES (
      r.aso_client_id, r.owner_id, r.amount, 'reversal_withdrawal_fee', r.payment_method,
      'completed',
      'Refund: fee was wrongly charged against a group/esusu withdrawal using an unrelated personal-cycle commission rate (system fix 2027-01-09)',
      NULL,
      r.id, 'completed',
      r.cycle_id, r.group_id, r.contribution_context
    );

    UPDATE aso_clients
    SET current_balance = COALESCE(current_balance, 0) + r.amount
    WHERE id = r.aso_client_id;

    v_fixed := v_fixed + 1;
    v_total := v_total + r.amount;
  END LOOP;

  RAISE NOTICE 'group/esusu withdrawal-fee refund: reversed % row(s), total refunded ₦%', v_fixed, v_total;
END $$;
