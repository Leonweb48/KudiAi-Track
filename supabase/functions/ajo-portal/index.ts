...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "initialize-payment") {
      // Retired — client contributions/deposits are wallet-only now (see
      // pay-contribution-wallet). Kept as a hard block, not a removed route,
      // so a stale cached client build can't silently start a new Paystack
      // checkout and leave another orphaned 'pending' contribution behind —
      // exactly what happened once already when this path was still live.
      return json({ error: "Card/bank payments have been retired — please pay from your KudiAI Wallet instead." }, 410);
    }

    // ── Confirm a Paystack contribution payment (client self-pay) — kept live
    //    (not a source of new stuck rows) so any pre-existing pending Paystack
    //    contribution can still be resolved if it genuinely did succeed. ──────
