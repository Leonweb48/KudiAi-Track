...
const ORG_SELECT = `id, owner_id, name, type, reg_number, description, address, state_name, lga, phone, email, logo_url,
...
const MEMBER_SELECT = `id, org_id, membership_id, full_name, email, phone, role, status,
  profile_image_url, address, occupation, date_of_birth, joined_date,
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "create-org") {
      const { owner_id, name, type, description, address, state_name, lga, phone, email } = body as Record<string,string>;
      if (!owner_id || !name || !type) return json({ error: "owner_id, name, and type are required" }, 400);

      // Generate unique reg number
      const prefix = type === "cooperative" ? "COOP" : type === "church" ? "CHU" : type === "ngo" ? "NGO"
        : type === "market_association" ? "MKT" : type === "youth_group" ? "YTH" : "SAV";
      const reg_number = `${prefix}-${Date.now().toString(36).toUpperCase().slice(-6)}`;

      const { data: org, error } = await sb.from("organizations")
        .insert({ owner_id, name, type, reg_number, description, address, state_name, lga, phone, email })
    ...
    if (action === "update-org") {
      ...
      const allowed = ["name","description","address","state_name","lga","phone","email","logo_url","status"];
    ...
    if (action === "add-member") {
      const { org_id, full_name, email, phone, role, address, occupation } = body as Record<string,string>;
      if (!org_id || !full_name) return json({ error: "org_id and full_name required" }, 400);

      // Count existing to generate sequential ID
      const { count } = await sb.from("org_members").select("id", { count: "exact", head: true })
        .eq("org_id", org_id).neq("status", "removed");
      const seq = String((count || 0) + 1).padStart(4, "0");

      const { data: orgRow } = await sb.from("organizations").select("name,reg_number").eq("id", org_id).single();
      ...
      const { data: member, error } = await sb.from("org_members")
        .insert({ org_id, membership_id, full_name, email, phone, role: role || "member", address, occupation })
        .select(MEMBER_SELECT).single();
      if (error) return json({ error: error.message }, 400);

      await sb.from("organizations").update({ member_count: (count || 0) + 1 }).eq("id", org_id);
      return json({ member });
    }
    ...
    if (action === "get-members") {
      ...
      const { data: members } = await q.order("created_at", { ascending: false });
    ...
    if (action === "get-member") {
      ...
      const [{ data: savings }, { data: loans }] = await Promise.all([
        sb.from("org_savings").select("*").eq("member_id", member_id).order("created_at", { ascending: false }).limit(20),
        sb.from("org_loans").select("*").eq("member_id", member_id).order("created_at", { ascending: false }),
      ]);
      return json({ member, savings: savings || [], loans: loans || [] });
    }
    ...
    if (action === "update-member") {
      ...
      const allowed = ["full_name","email","phone","role","status","address","occupation",
                       "suspension_reason","suspended_at","removed_at","portal_pin"];
      const update: Record<string, unknown> = {};
      for (const k of allowed) if (fields[k] !== undefined) update[k] = fields[k];
    ...
    if (action === "record-saving") {
      const { org_id, member_id, amount, type, payment_method, paystack_ref, notes, recorded_by } =
      ...
      const { data: saving, error } = await sb.from("org_savings")
        .insert({ org_id, member_id, amount: amt, type: type || "deposit", payment_method: payment_method || "cash",
          paystack_ref, notes, recorded_by, balance_after })
        .select("*").single();
      if (error) return json({ error: error.message }, 400);
      ...
      await sb.from("organizations").update({
        total_savings: Math.max(0, (await sb.from("organizations").select("total_savings").eq("id", org_id).single()).data?.total_savings + delta),
        wallet_balance: Math.max(0, (await sb.from("organizations").select("wallet_balance").eq("id", org_id).single()).data?.wallet_balance + delta),
      }).eq("id", org_id);

      // Wallet transaction
      const { data: orgW } = await sb.from("organizations").select("wallet_balance").eq("id", org_id).single();
      await sb.from("org_wallet_txns").insert({
        ...
        reference_id: saving.id, paystack_ref, balance_after: orgW?.wallet_balance || 0,
      });
    ...
    if (action === "get-savings") {
      ...
      let q = sb.from("org_savings").select("*, org_members(full_name,membership_id)");
    ...
    if (action === "apply-loan") {
      ...
      const { data: loan, error } = await sb.from("org_loans")
        .insert({ org_id, member_id, amount_requested: parseFloat(String(amount_requested)),
          ...
          loan_purpose, repayment_months: parseInt(String(repayment_months || 1)), notes })
    ...
    if (action === "update-loan") {
      const { loan_id, org_id, status, amount_approved, approved_by_name, notes } = body as Record<string, unknown>;
      ...
      if (status === "approved") {
        ...
        update.outstanding_balance = update.amount_approved || amount_approved;
      }
      if (status === "disbursed") {
        update.disbursed_at = new Date().toISOString();
        const months = parseInt(String((body as Record<string,unknown>).repayment_months || 1));
        ...
        if (amt > 0) {
          const { data: orgW } = await sb.from("organizations").select("wallet_balance,total_loans_out").eq("id", org_id).single();
    ...
    if (action === "record-repayment") {
      const { org_id, loan_id, member_id, amount, payment_method, paystack_ref, notes } = body as Record<string, unknown>;
      const amt = parseFloat(String(amount));
      if (!loan_id || !amt) return json({ error: "loan_id and amount required" }, 400);

      const { data: loan } = await sb.from("org_loans").select("outstanding_balance,status").eq("id", loan_id).single();
      if (!loan) return json({ error: "Loan not found" }, 404);

      const newOutstanding = Math.max(0, (loan.outstanding_balance || 0) - amt);
      const newStatus = newOutstanding === 0 ? "repaid" : "disbursed";

      const { data: repayment, error } = await sb.from("org_loan_repayments")
        .insert({ org_id, loan_id, member_id, amount: amt, payment_method: payment_method || "cash", paystack_ref, notes })
        .select("*").single();
      if (error) return json({ error: error.message }, 400);

      await sb.from("org_loans").update({ outstanding_balance: newOutstanding, status: newStatus }).eq("id", loan_id);
      ...
      await sb.from("org_wallet_txns").insert({
        ...
        paystack_ref, balance_after: newBal,
      });

      return json({ repayment, outstanding_balance: newOutstanding, loan_status: newStatus });
    }
    ...
    if (action === "create-meeting") {
      const { org_id, title, description, meeting_type, scheduled_at, location } = body as Record<string, string>;
      ...
      const { data: meeting, error } = await sb.from("org_meetings")
        .insert({ org_id, title, description, meeting_type: meeting_type || "general", scheduled_at, location })
        .select("*").single();
      if (error) return json({ error: error.message }, 400);
      return json({ meeting });
    }
    ...
    if (action === "update-meeting") {
      ...
      const allowed = ["title","description","meeting_type","scheduled_at","location","status"];
      const update: Record<string, unknown> = {};
      for (const k of allowed) if (fields[k] !== undefined) update[k] = fields[k];
      const { data: meeting, error } = await sb.from("org_meetings").update(update)
        .eq("id", meeting_id).select("*").single();
      if (error) return json({ error: error.message }, 400);
      return json({ meeting });
    }
    ...
    if (action === "record-attendance") {
      const { meeting_id, org_id, member_id, status } = body as Record<string, string>;
      if (!meeting_id || !org_id || !member_id) return json({ error: "meeting_id, org_id, member_id required" }, 400);
      const { data: att, error } = await sb.from("org_attendance")
        .upsert({ meeting_id, org_id, member_id, status: status || "present", checked_in_at: new Date().toISOString() },
    ...
    if (action === "bulk-attendance") {
      ...
      if (!rows.length) return json({ count: 0 });
      const { error } = await sb.from("org_attendance")
        .upsert(rows, { onConflict: "meeting_id,member_id" });
      if (error) return json({ error: error.message }, 400);

      // Mark meeting completed if bulk-submitting
    ...
    if (action === "qr-check-in") {
      ...
      if (meeting.status === "cancelled") return json({ error: "This meeting has been cancelled" }, 400);
    ...
    if (action === "get-wallet") {
      ...
      const { data: org } = await sb.from("organizations").select("wallet_balance,total_savings,total_loans_out").eq("id", org_id).single();
      return json({ transactions: txns || [], wallet: org });
    }

    if (action === "receive-contribution") {
      const { org_id, amount, description, paystack_ref } = body as Record<string, unknown>;
      const amt = parseFloat(String(amount));
      if (!org_id || !amt) return json({ error: "org_id and amount required" }, 400);
      const { data: orgW } = await sb.from("organizations").select("wallet_balance").eq("id", org_id).single();
      const newBal = (orgW?.wallet_balance || 0) + amt;
      await sb.from("organizations").update({ wallet_balance: newBal }).eq("id", org_id);
      await sb.from("org_wallet_txns").insert({
        org_id, type: "contribution", amount: amt, description: description || "Contribution received",
        paystack_ref, balance_after: newBal,
      });
    ...
    if (action === "member-auth") {
      ...
      let q = sb.from("org_members").select(`${MEMBER_SELECT}, organizations(id,name,type,logo_url)`)
    ...
    if (action === "member-by-token") {
      ...
      const { data: member } = await sb.from("org_members")
        .select(`${MEMBER_SELECT}, organizations(id,name,type,logo_url,phone,email)`)
    ...
    if (action === "member-get-savings") {
      const { member_id } = body as { member_id: string };
      const { data: savings } = await sb.from("org_savings").select("*")
        .eq("member_id", member_id).order("created_at", { ascending: false }).limit(50);
      return json({ savings: savings || [] });
    }

    if (action === "member-get-loans") {
      const { member_id } = body as { member_id: string };
      const { data: loans } = await sb.from("org_loans").select("*")
        .eq("member_id", member_id).order("created_at", { ascending: false });
      const { data: repayments } = await sb.from("org_loan_repayments").select("*")
        .eq("member_id", member_id).order("created_at", { ascending: false });
      return json({ loans: loans || [], repayments: repayments || [] });
    }

    if (action === "member-request-loan") {
      const { org_id, member_id, amount_requested, interest_rate, loan_purpose, repayment_months } =
        body as Record<string, unknown>;
      if (!org_id || !member_id || !amount_requested) return json({ error: "Required fields missing" }, 400);
      const { data: loan, error } = await sb.from("org_loans").insert({
        org_id, member_id, amount_requested: parseFloat(String(amount_requested)),
        interest_rate: parseFloat(String(interest_rate || 0)),
        loan_purpose, repayment_months: parseInt(String(repayment_months || 1)),
      }).select("*").single();
      if (error) return json({ error: error.message }, 400);
      return json({ loan });
    }
    ...
    if (action === "member-get-meetings") {
      ...
      const { data: attendance } = await sb.from("org_attendance").select("meeting_id,status")
        .eq("member_id", member_id);
      const attMap: Record<string, string> = {};
      (attendance || []).forEach((a: Record<string, string>) => { attMap[a.meeting_id] = a.status; });
      return json({ meetings: meetings || [], attendance_map: attMap });
    }
